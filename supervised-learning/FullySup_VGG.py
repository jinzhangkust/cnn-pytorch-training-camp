...
import numpy as np
## 导入数据集
from data import get_froth_data_2
## 导入评价指标
...
## 从torchvision模型库中导入VGG网络
vggmodel = torchvision.models.vgg16(weights=None)
## 修改VGG网络输出端，使之适应泡沫图像分类
class LinearSensor(nn.Module):
    def __init__(self):
        super(LinearSensor, self).__init__()
        self.feature = vggmodel.features  ##VGG网络的特征提取模块
        self.feature.add_module('pool', nn.AdaptiveAvgPool2d((1, 1)))
        self.predictor = nn.Sequential(
            nn.Dropout(p=0.2),  # add before FC
            nn.Linear(512, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 5))  ## N分类,则输出通道数设为N，我这里的数据集是5分类

        for m in self.modules():  ## CNN网络参数初始化
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight, gain=1.0)

    def forward(self, x):
        x = self.feature(x)  ## 特征编码
        x = self.predictor(x.view(x.size(0), -1))  ## 基于特征预测所属类别
        return x


def set_loader(opt):  ## 加载数据
    train_data, val_data, test_data = get_froth_data_2()
...
def set_model(opt):  ## 加载m模型
    model = LinearSensor().cuda()  ##定义模型为LinearSensor
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1).cuda()  ##定义损失函数为交叉熵
    cudnn.benchmark = True
    if opt.resume:  ## 从断点恢复训练
...
def set_optimizer(opt, model):  ##定义优化器
    optimizer = optim.Adam(model.parameters(), lr=opt.learning_rate, weight_decay=opt.weight_decay)
    return optimizer

def cal_metrics(pred, true):  ##分类指标计算
...
def train(train_loader, model, criterion, optimizer, epoch, opt, tb):
    ...
    for idx, (_, images, targets) in enumerate(train_loader):
        ...
        bsz = targets.shape[0]
        # compute loss
        output = model(images)  ##模型预测输出
        loss = criterion(output, targets)  ##计算输出与目标的交叉熵
        # SGD
        optimizer.zero_grad()  ##清零模型参数的梯度
        loss.backward()  ##计算损失相较于模型参数的梯度
        optimizer.step()  ##梯度下降优化模型参数
        # update metric
        _, pred = torch.max(output, 1)  ##模型N通道输出转为标量硬标签
